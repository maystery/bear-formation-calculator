...
test('allocate water-fills marches without exceeding caps', () => {
  assert.deepEqual(
    allocate(705730, [169310, 169310, 169310, 98900, 98900]),
...
test('allocate fills higher-priority hero rallies before no-hero rallies', () => {
  assert.deepEqual(allocate(300000, [169310, 169310, 98900], [1, 1, 0]), [150000, 150000, 0]);
  assert.deepEqual(allocate(400000, [169310, 169310, 98900], [1, 1, 0]), [169310, 169310, 61380]);
});
...
test('allocateSequentially fills each march before moving to the next', () => {
  assert.deepEqual(allocateSequentially(300000, [169310, 169310, 98900]), [169310, 130690, 0]);
  assert.deepEqual(allocateSequentially(400000, [80000, Infinity, 98900]), [80000, 320000, 0]);
});

test('splitByRatio always sums to its requested total', () => {
  const split = splitByRatio(2, { inf: 0.25, cav: 0.25, arc: 0.5 });
...
test('splitMarches reconciles global stock rounding', () => {
  ...
  const rows = splitMarches(totals, { inf: 0.1, cav: 0.1, arc: 0.8 });
  assert.deepEqual(
    rows.map((row) => row.inf + row.cav + row.arc),
    totals,
  );
  assert.equal(
    rows.reduce((sum, row) => sum + row.arc, 0),
    1000,
  );
  assert.equal(
    rows.reduce((sum, row) => sum + row.inf, 0),
    125,
  );
  assert.equal(
    rows.reduce((sum, row) => sum + row.cav, 0),
    125,
  );
});
