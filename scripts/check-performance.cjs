...
for (const engine of ['chromium', 'webkit']) {
  browserSuite(
    `Performance: ${engine}`,
    { engine, context: { viewport: { width: 390, height: 844 }, isMobile: true, hasTouch: true } },
    (test) => {
      test('portrait preloading, no duplicate downloads or layout shift, visibility and reduced motion', async ({
        page,
        url,
      }) => {
        const requests = [];
        ...
        await page.evaluate(() => {
          ...
          document.querySelectorAll('.hero-card__image').forEach((image) => {
            image.addEventListener('load', () =>
              window.loadedPortraits.push(image.getAttribute('href')),
            );
          });
        });
        ...
        const firstTop = await card.evaluate(
          (element) => element.getBoundingClientRect().top + scrollY,
        );
        await page.evaluate((top) => scrollTo(0, top - innerHeight - 200), firstTop);
        await page.waitForFunction(() =>
          window.loadedPortraits.includes('assets/heroes/weeandwoo.webp'),
        );
        assert.ok(
          await card.evaluate((element) => element.getBoundingClientRect().top > innerHeight),
          'portrait should preload before reaching the visible viewport',
        );
        assert.equal(
          await card.evaluate((element) => getComputedStyle(element).animationPlayState),
          'paused',
        );
        assert.equal(
          await page.locator('[data-hero="hilde"] .hero-card__image').getAttribute('href'),
          null,
        );
        assert.deepEqual(
        ...
        await page.waitForFunction(
          () =>
            getComputedStyle(document.querySelector('[data-hero="weeWoo"]')).animationPlayState ===
            'running',
        );
        assert.equal(
          await card.evaluate(
      ...
      test('unchanged controls have zero mutations; hero, march and capacity changes still refresh correctly', async ({
        page,
        url,
      }) => {
        await page.goto(url);
        ...
        await page.evaluate(() => {
          ...
          for (const [key, id] of [
            ['heroes', 'heroGrid'],
            ['buffs', 'capacityBuffsTitle'],
            ['valora', 'valoraSkillMenu'],
            ['bison', 'bisonSkillMenu'],
          ]) {
            const node =
              key === 'buffs'
        ...
        await nextPaint(page);
        assert.deepEqual(await page.evaluate(() => window.uiMutations), {
          heroes: 0,
          buffs: 0,
          valora: 0,
          bison: 0,
        ...
        assert.equal(
          await page.evaluate(
            () => window.uiMutations.heroes + window.uiMutations.valora + window.uiMutations.bison,
          ),
          0,
        );
        await page.evaluate(() => {
          document.getElementById('capacityEdit').open = true;
        });
      ...
      test('closed popups do no scroll work; open tooltip positioning batches into one frame', async ({
        page,
        url,
      }) => {
        await page.goto(url);
        ...
        const trigger = page.locator(
          '[data-hero="weeWoo"] .hero-stat--attack.hero-stat--interactive',
        );
      ...
      for (const fallback of [false, true]) {
        test(
          fallback
            ? 'portraits load without IntersectionObserver'
            : 'collapsed portraits load on expansion',
          async ({ page, url }) => {
