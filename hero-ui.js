(function (root, factory) {
  ...
  const STAT_TYPES = Object.freeze({
    attack: Object.freeze({
      label: 'Attack',
      icon: 'assets/skills/attack.webp',
      description: 'Increases Squad Attack.',
    }),
    lethality: Object.freeze({
      label: 'Lethality',
      icon: 'assets/skills/leathality.webp',
      description: 'Increases Squad Lethality.',
    }),
    defense: Object.freeze({
      label: 'Defense',
      icon: 'assets/skills/def.webp',
      description: 'Increases Squad Defense.',
    }),
    enemyDamageTaken: Object.freeze({
      label: 'Enemy DMG Taken',
      icon: 'assets/skills/enemy_dmg_taken.webp',
      description: 'Increases damage taken by enemy squads.',
    }),
    enemyDefense: Object.freeze({
      label: 'Enemy DEF',
      icon: 'assets/skills/enemy_def.webp',
      description: 'Reduces enemy squad Defense.',
    }),
  });

  const RECOMMENDED_LEVEL_ICON = 'assets/skills/min_lvl.webp';
  ...
  const HERO_ARTBOARD = Object.freeze({
    ...
    height: 260,
    breakoutCanvas: 50,
    leftBreakoutCanvas: 50,
    radius: 16,
  });

  // Each expedition skill owns its recommended level; it is guidance, not an eligibility gate.
  const HEROES = Object.freeze({
    none: {
      name: 'No hero',
      avatar: 'assets/heroes/profile/none.webp',
    },
    amadeus: {
      name: 'Amadeus',
      portrait: 'assets/heroes/amadeus.webp',
      avatar: 'assets/heroes/profile/amadeus.webp',
      inputId: 'amaOn',
      season: 1,
    ...
    chenko: {
      ...
      avatar: 'assets/heroes/profile/chenko.webp',
      inputId: 'chenkoOn',
      season: 1,
      deployCap: 'Hero deploy cap',
      expeditionSkill: {
    ...
    yeonwoo: {
      ...
      avatar: 'assets/heroes/profile/yeonwoo.webp',
      inputId: 'yeonwooOn',
      season: 1,
      deployCap: 'Hero deploy cap',
      expeditionSkill: {
    ...
    amane: {
      ...
      avatar: 'assets/heroes/profile/amane.webp',
      inputId: 'amaneOn',
      season: 1,
      deployCap: 'Hero deploy cap',
      expeditionSkill: {
    ...
    margot: {
      ...
      avatar: 'assets/heroes/profile/margot.webp',
      inputId: 'margotOn',
      season: 4,
      deployCap: 'Hero deploy cap',
      expeditionSkill: {
    ...
    vivian: {
      ...
      avatar: 'assets/heroes/profile/vivian.webp',
      inputId: 'vivianOn',
      season: 5,
      deployCap: 'Hero deploy cap',
      expeditionSkill: {
    ...
    ava: {
      ...
      avatar: 'assets/heroes/profile/ava.webp',
      inputId: 'avaOn',
      season: 7,
      deployCap: 'Hero deploy cap',
      expeditionSkill: {
    ...
    weeWoo: {
      name: 'Wee & Woo',
      portrait: 'assets/heroes/weeandwoo.webp',
      avatar: 'assets/heroes/profile/wee-woo.webp',
      inputId: 'weeWooOn',
      season: 7,
      deployCap: 'Hero deploy cap',
      expeditionSkill: {
    ...
    hilde: {
      ...
      avatar: 'assets/heroes/profile/hilde.webp',
      inputId: 'hildeOn',
      season: 2,
      deployCap: 'Hero deploy cap',
      expeditionSkill: {
  ...
  function statType(stat, interactive = false, attributes = '') {
    ...
    if (!type) return '';
    return (
      `<span class="hero-stat hero-stat--${stat}${interactive ? ' hero-stat--interactive' : ''}" data-stat="${stat}"` +
  ...
  /** @param {{values: number[]}} effect
   * @param {number} recommendedLevel
   * @param {number | null} [selectedLevel]
    ...
    return effect.values
      .map((value, index) => {
        ...
        const states = [
          ...
          value === recommendedValue ? 'is-recommended-value' : '',
        ]
          .filter(Boolean)
          .join(' ');
        const labels = [
          level === recommendedLevel ? 'recommended level' : '',
          level === effectiveSelectedLevel ? 'selected level' : '',
          value === recommendedValue ? 'recommended value' : '',
        ]
          .filter(Boolean)
          .join(', ');
        return (
      ...
      .map((effect) => {
        ...
        const effectMarkup =
          ...
          `<span class="skill-levels">${skillLevelRows(
            ...
            skill.selectedLevel,
            RECOMMENDED_STAT_VALUES[effect.stat],
          )}</span></span>`;
        return (
  ...
  function renderStatLegend() {
    const items = Object.entries(STAT_TYPES)
      .map(
        ([key, type]) =>
          `<span class="stat-legend__item" title="${type.description}" ` +
          `aria-label="${type.label}: ${type.description}">${statType(key)}</span>`,
      )
      .join('');
    return (
      `<aside class="stat-legend" aria-labelledby="stat-legend-title">` +
      `<strong class="stat-legend__title" id="stat-legend-title">Stat types</strong>` +
      `<span class="stat-legend__items">${items}</span></aside>`
    );
  }

  function makePortraitClipPath({
    start,
    end,
    height,
    shoulder,
    ...
    if (unrestricted) {
      return (
        `M 0 ${-breakoutCanvas} L ${W} ${-breakoutCanvas} ` +
        `L ${W} ${H - R} Q ${W} ${H} ${W - R} ${H} ` +
        `L ${R} ${H} Q 0 ${H} 0 ${H - R} Z${leftOpening}`
      );
    }
    const leftShoulderLow = start + shoulder * 0.35;
    const leftShoulderHigh = start + shoulder * 0.65;
    const rightShoulderHigh = end - shoulder * 0.65;
    const rightShoulderLow = end - shoulder * 0.35;
    return (
      `M ${R} 0 L ${start} 0 ` +
  ...
  function renderHeroPriority() {
    const escapeHtml = (value) =>
      value
        ...
        .replaceAll("'", '&#39;');
    return (
      '<span class="hero-priority-label">Assignment priority</span>' +
      '<span class="hero-priority-list" role="list">' +
      HERO_SLOTS.map(
        ({ key }, index) =>
          `<span class="hero-priority-chip" role="listitem">` +
          `<b>${index + 1}</b><img class="hero-priority-avatar" src="${escapeHtml(HEROES[key].avatar)}" alt="" width="24" height="24"><span>${escapeHtml(HEROES[key].name)}</span></span>`,
      ).join('') +
      '</span>'
    );
  }
